          ...
          <el-card class="gauge-card">
            ...
            <div class="gauge-content">
              ...
              <div class="gauge-info">
                <div
                    v-for="item in depositTypeData.items"
                    :key="item.dictLabel"
                    class="info-item"
                >
                  <span class="info-label">{{ item.dictLabel }}：</span>
                  <span class="info-value">¥{{ formatMoney(item.mny || 0) }}</span>
                </div>
                <div class="info-item">
      ...
      <el-row :gutter="20" class="analysis-row">
        ...
        <el-col :span="12">
          <el-card class="analysis-card">
            <template #header>
              <div class="card-header">
                <span class="header-title">🤖 AI财务分析</span>
              </div>
            </template>
            <div class="ai-analysis">
              <div class="input-area">
                ...
                <el-button
                    ...
                    class="submit-btn"
                >
                  <span v-if="!aiLoading">🤖 分析</span>
                  <span v-else>思考中...</span>
                </el-button>
              </div>
              <div v-if="aiAnalysisResult" class="result-area">
              ...
              <div v-if="!aiAnalysisResult && !aiLoading" class="placeholder-text">
                请输入您的财务分析问题，AI将为您进行专业的财务分析。
              </div>
            </div>
          </el-card>
        </el-col>
...
import { getLastSummaryDetailByType, getAIFinancialAnalysis } from '@/api/main/summary'
...
// 从后端获取存款类型数据
const fetchDepositTypeData = async () => {
  try {
    const response = await getLastSummaryDetailByType();
    if (response.code === 200) {
      // 将返回的数据按照类型映射到存款类型
      const data = response.data || [];

      // 计算总金额
      const total = data.reduce((sum, item) => sum + (item.mny || 0), 0);

      // 存储所有类型的数据
      depositTypeData.value = {
        items: data,
        total: total
      };
    }
  } catch (error) {
    console.error('获取存款类型数据失败:', error);
  }
};
...
const handleAISubmit = async () => {
  if (!aiQuery.value.trim()) {
    ElMessage.warning('请输入您的财务分析问题');
    return;
  }

  aiLoading.value = true;
  try {
    const response = await getAIFinancialAnalysis(aiQuery.value);
    if (response.code === 200) {
      debugger
      aiAnalysisResult.value = response.msg;
...
const renderedMarkdown = computed(() => {
  if (!aiAnalysisResult.value) return '';
  // 使用marked将Markdown转换为HTML，然后使用DOMPurify清理HTML以防止XSS
  const rawHtml = marked(aiAnalysisResult.value);
  return DOMPurify.sanitize(rawHtml);
});

// 全屏切换功能
const toggleFullscreen = () => {
  const element = document.querySelector('.ai-analysis');
...
const getChartOption = (data, showPredictionData = true) => {
  ...
  // 如果显示预测且有预测数据，使用后端返回的预测存款数据
  if (showPredictionData) {
    const predictedData = data.map(item => item.predictedDeposit);
  ...
  const margin = range > 0 ? range * 0.1 : 10000; // 10%的边距，如果range为0则使用固定边距
  const dynamicMin = Math.max(0, minValue - margin); // 确保最小值不小于0
...
const initPieChart = () => {
  ...
  const colorPalette = [
    ...
    '#13c2c2', '#ff7a45', '#eb2f96', '#597ef7', '#69c0ff',
    '#49aa19', '#95de64', '#d3adf7', '#ffd666', '#ffadd2'
  ];
    ...
    series: [
      {
        ...
        data: depositTypeData.value.items.map((item, index) => ({
          value: item.mny || 0,
          name: item.dictLabel,
          itemStyle: {
